...
class InferenceOrchestratorService:
    ...
    def _get_model(self, variant: ModelVariant):
        ...
        with self._sync_load_locks[variant]:
            ...
            if variant == ModelVariant.YOLO_STANDARD:
                model = YoloWorldWrapper()
                model.load()
                self._models[variant] = model
                
            elif variant == ModelVariant.YOLO_SAHI:
                model = YoloWorldSahiSahiWrapper()
                model.load()
                self._models[variant] = model
    ...
    async def process_detection(
        ...
        try:
            result = await self.image_analysis(
